...
function show_objective_function_summary(io::IO, model::Model)
    nonlinear_objective = _nlp_objective_function(model)
...
function objective_function_string(mode, model::Model)
    ...
    return nonlinear_expr_string(model, mode, nonlinear_objective)
end
...
"""
    nonlinear_expr_string(model::Model, mode::MIME, c::NonlinearExprData)
